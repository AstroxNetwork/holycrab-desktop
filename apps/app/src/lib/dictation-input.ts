export interface DictationInsertionResult {
  nextText: string
  nextCursor: number
}
...
export function computeDictationInsertion(
  ...
  const needsPrefixSpace = Boolean(
    beforeChar
    && !/\s/.test(beforeChar)
    && firstChar
  ...
  const needsSuffixSpace = Boolean(
    afterChar
    && !/\s/.test(afterChar)
    && lastChar
    && isWordChar(lastChar)
  ...
  const insertText = `${needsPrefixSpace ? ' ' : ''}${transcriptText}${needsSuffixSpace ? ' ' : ''}`
